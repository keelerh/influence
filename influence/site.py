...
class Site(object):
    ...
    def __init__(self, label: str, s: np.ndarray):
        ...
        Within each site is an m-status Markov chain Γ(A) where A is an m x m stochastic matrix. At any given time, the
        site is in one of the statuses of Γ(A). The status of the site evolves with transition probabilities that depend
        on its current status and the current statuses of its surrounding neighbors.
